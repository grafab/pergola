...
We create two new maps from the same data using another method calles `order.endlink` from the gclus package.
We use it both on the global data set and on the linkage groups
```{r}
...
First we have a look at the order again.
```{r, fig.show = 'hold', fig.width = 10, fig.height = 10}
...
Now, we create dendrograms from our maps and compare them with each other.
...
We create a dendogram list and visualize it using `maketangle`.
```{r, fig.show = 'hold', fig.width = 10, fig.height = 15}
