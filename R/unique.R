...
allTrees <- function(hc, dis, start = 1) {
  ...
  for (i in (start + 1):nrow(hc$merge)) {
    ...
    heights <- apply(pairs, 1, function(x)
      dis[x[1], x[2]])
    ...
    if (sum(sheights) > 1) {
      for (j in which(sheights)) {
        hc2 <-
          switchEntries(
            hc,
            e1 = getNode(hc$merge, pairs[j, 1], max = i),
...
sarfExt <- function(uniOrd,  dis, maxSarf = length(uniOrd[1,]) - 1) {
  ...
  if (is.vector(uniOrd)) {
    return(uniOrd)
  } else if (nrow(uniOrd) == 2 &&
             all(uniOrd[1,] == rev(uniOrd[2,]))) {
    ...
    warning(
      paste(
        "Multiple orders with same SARF score, only top one returned. Increase of maxSarf(",
        maxSarf,
        ") might help.",
        sep = ""
      )
